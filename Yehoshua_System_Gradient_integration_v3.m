function results = Yehoshua_System_Gradient_integration_v3()
% Yehoshua&System_Gradient_integration
%
% Save this file exactly as:
%   Yehoshua_System_Gradient_integration_v3.m
%
...
% artifact is created. The only written artifact is one PNG image.
    ...
    fprintf('\n============================================================\n');
    fprintf(' Yehoshua&System_Gradient_integration v3\n');
    fprintf(' SNR-adaptive gate + temporal noise + adaptive topology\n');
    ...
    finalRGB = composeSingleImage( ...
        I0_RGB, IA_RGB, base, optical, v1, quant, ...
...
function cfg = configuration()
    ...
    cfg.version = ...
        'Yehoshua_System_Gradient_integration_v3_noise_adaptive';
...
function photo = simulatePhototransduction(Q, cfg)
    ...
    % Online temporal statistics: two additional 2-D fields only.
    ...
    for stepIndex = 1:cfg.photo.steps
        ...
        % Welford online temporal mean and variance.
...
function finalRGB = composeSingleImage( ...
    ...
    qGateRGB = cat( ...
        3, ...
        normalize01(qGate.overload, cfg.robustSampleCount), ...
        qGate.integratedOutput, ...
        qGate.precision);
    ...
    finalRGB = ...
        single(0.23) * IA_RGB + ...
        single(0.15) * I0_RGB + ...
        single(0.22) * gradientRGB .* gateRGB + ...
        single(0.09) * coneRGB .* gateRGB + ...
        single(0.08) * phaseRGB .* gateRGB + ...
        single(0.11) * qGateRGB .* gateRGB + ...
        single(0.12) * networkRGB .* ...
        (single(0.35) + ...
        single(0.65) * coherentGateRGB);
    ...
    finalRGB = ...
        finalRGB + ...
        single(0.04) * ...
        oppositionRGB .* gateRGB;

    finalRGB = ...
        finalRGB .* ...
        (single(0.86) + ...
        single(0.08) * precisionRGB + ...
        single(0.06) * confidenceRGB);

    finalRGB(:, :, 1) = ...
        finalRGB(:, :, 1) + ...
        single(0.14) * quant.coreGlow + ...
        single(0.12) * network.semanticMap;

    finalRGB(:, :, 2) = ...
        finalRGB(:, :, 2) + ...
        single(0.22) * quant.coreGlow + ...
        single(0.16) * network.edgeMap;

    finalRGB(:, :, 3) = ...
        finalRGB(:, :, 3) + ...
        single(0.30) * quant.coreGlow + ...
        single(0.20) * network.activityMap;
