...
% a new timestamped output directory.
    ...
    logPath = fullfile(runDir, 'qnt_for_qnt_run_log.txt');
    diary(logPath);
    ...
    t = tic;
    [I0, channelWeights, ~] = integrateChannels(channelCube, cfg);
...
function cfg = defaultConfiguration()
    cfg = struct();
    cfg.version = 'Quantization_of_Knowledge_For_The_Quantization_of_Knowledge_v1';
    ...
    cfg.outputRoot = '/MATLAB Drive/modelTRAINING/qnt_for_qnt_output';
...
%% ========================================================================
function cores = calibrateHumanSymbols(cores, state, ~)
...
function paths = writeArtifacts(runDir, I0_RGB, IA_RGB, masterRGB, I0, base, multiscale, ...
    ...
    paths.masterRaw = fullfile(runDir, '03_qnt_for_qnt_master_raw.png');
    paths.masterAnnotated = fullfile(runDir, '04_qnt_for_qnt_master_annotated.png');
    paths.contactSheet = fullfile(runDir, '05_qnt_for_qnt_contact_sheet.png');
    paths.derivedH5 = fullfile(runDir, 'qnt_for_qnt_derived_fields.h5');
    paths.metricsJSON = fullfile(runDir, 'qnt_for_qnt_metrics.json');
    paths.coresCSV = fullfile(runDir, 'qnt_for_qnt_cores.csv');
    paths.relationsCSV = fullfile(runDir, 'qnt_for_qnt_relations.csv');
    paths.chainTXT = fullfile(runDir, 'qnt_for_qnt_chain.txt');

    imwrite(toUint8(I0_RGB), paths.sourceRGB);
    imwrite(toUint8(IA_RGB), paths.displayRGB);
    imwrite(toUint8(masterRGB), paths.masterRaw);

    renderAnnotatedMaster(masterRGB, knowledge.cores, graphState.edges, state, ...
        paths.masterAnnotated, cfg);

    try
        renderContactSheet(I0_RGB, IA_RGB, masterRGB, I0, base, multiscale, spectral, ...
            optical, photo, retina, lgn, v1, knowledge, state, paths.contactSheet, cfg);
    catch contactSheetErr
        warning('qnt_for_qnt:contactSheetFailed', ...
            'Contact sheet could not be created (%s). Continuing without it so the rest of the run is not lost.', ...
            contactSheetErr.message);
    end
...
%% ========================================================================
function renderAnnotatedMaster(masterRGB, cores, edges, state, outputPath, cfg)
    fig = figure('Visible', 'off', 'Color', 'k', 'Position', [50, 50, 1300, 1300]);
    ...
    for e = 1:numel(edges)
        i = edges(e).source;
        j = edges(e).target;
        w = double(edges(e).weight);
        phaseValue = double(0.5 + 0.5 * cos(edges(e).phaseDifference));
        lineColor = [0.20 + 0.80 * w, 0.45 + 0.55 * phaseValue, 1.0];
        plot(ax, [cores(i).x, cores(j).x], [cores(i).y, cores(j).y], '-', ...
            'Color', min(lineColor, 1), 'LineWidth', 0.4 + 2.2 * w);
    end

    if ~isempty(cores)
        positive = [cores.winding] >= 0;
        x = [cores.x];
        y = [cores.y];
        sizes = cfg.render.coreMarkerSize * (0.45 + normalizeVector([cores.amplitude]));

        scatter(ax, x(positive), y(positive), sizes(positive), 'o', ...
            'MarkerEdgeColor', [0.75, 1.00, 1.00], 'LineWidth', 1.0);
        scatter(ax, x(~positive), y(~positive), sizes(~positive), 's', ...
            'MarkerEdgeColor', [1.00, 0.72, 0.95], 'LineWidth', 1.0);
        ...
        for k = 1:labelCount
            i = labelOrder(k);
            text(ax, cores(i).x + 5, cores(i).y - 5, cores(i).symbol, ...
                'Interpreter', 'tex', 'Color', [1, 1, 1], 'FontSize', 12, ...
                'FontWeight', 'bold', 'HorizontalAlignment', 'left', ...
                'VerticalAlignment', 'bottom');
        end
    end

    titleText = sprintf(['Quantization of Knowledge   |   cores=%d   relations=%d   ', ...
        'R_{\\Phi}=%.4f   I_{K}=%.4f'], numel(cores), numel(edges), ...
        state.phaseOrderFinal, state.integrationIndexFinal);
    text(ax, 12, 28, titleText, 'Interpreter', 'tex', 'Color', [0.92, 0.98, 1.00], ...
        'FontSize', 13, 'FontWeight', 'bold', 'VerticalAlignment', 'top');

    exportgraphics(fig, outputPath, 'Resolution', cfg.render.exportDPI, 'BackgroundColor', 'black');
    close(fig);
end

%% ========================================================================
function renderContactSheet(I0_RGB, IA_RGB, masterRGB, I0, base, multiscale, spectral, ...
    optical, photo, retina, lgn, v1, knowledge, state, outputPath, cfg)

    fig = figure('Visible', 'off', 'Color', 'w', 'Position', [50, 50, 1800, 1350]);
    figCleanup = onCleanup(@() close(fig)); 
    layout = tiledlayout(fig, 4, 4, 'Padding', 'compact', 'TileSpacing', 'compact');

    showRGB(nexttile(layout), I0_RGB, 'I_0 RGB');
    showRGB(nexttile(layout), IA_RGB, 'I_A display operator');
    showScalar(nexttile(layout), I0, 'Canonical integrated field');
    showScalar(nexttile(layout), base.gradientMagnitude, '|nabla I_0|');

    showScalar(nexttile(layout), base.laplacian, 'Laplacian');
    showScalar(nexttile(layout), multiscale.dogEnergy, 'Multiscale DoG energy');
    showScalar(nexttile(layout), spectral.fftLogMagnitude, 'log(1+|FFT|)');
    showScalar(nexttile(layout), optical.energy, 'Optical energy field');

    showScalar(nexttile(layout), optical.Q(:, :, 1), 'q_L');
    showScalar(nexttile(layout), optical.Q(:, :, 2), 'q_M');
    showScalar(nexttile(layout), optical.Q(:, :, 3), 'q_S');
    showScalar(nexttile(layout), photo.meanVoltage, 'V_{photo} mean');

    showScalar(nexttile(layout), retina.on + retina.off, 'Retinal ON + OFF');
    showScalar(nexttile(layout), lgn.relay, 'Relay field');
    showScalar(nexttile(layout), v1.energy, 'V1 oriented energy');
    showRGB(nexttile(layout), masterRGB, sprintf('Master | K=%d | R_{Phi}=%.3f', ...
        numel(knowledge.cores), state.phaseOrderFinal));

    title(layout, 'Quantization of Knowledge - computational chain', ...
        'FontWeight', 'bold', 'FontSize', 17);

    drawnow;
    try
        exportgraphics(fig, outputPath, 'Resolution', cfg.render.exportDPI);
    catch exportErr
        warning('qnt_for_qnt:contactSheetExportFailed', ...
            'exportgraphics failed for the contact sheet (%s). Retrying with print().', ...
            exportErr.message);
        print(fig, outputPath, '-dpng', sprintf('-r%d', cfg.render.exportDPI));
    end
end

%% ========================================================================
function showRGB(ax, rgb, titleText)
    image(ax, rgb);
    axis(ax, 'image');
    axis(ax, 'off');
...
%% ========================================================================
function showScalar(ax, field, titleText)
    imagesc(ax, field);
    axis(ax, 'image');
    axis(ax, 'off');
    title(ax, titleText, 'Interpreter', 'tex');
