%% project_geodesic_velocity_VectorVision_UNIFIED_FIXED
...
PROJECT_NAME = 'project_geodesic_velocity_VectorVision_UNIFIED_FIXED';
...
for t = 0:T
    ...
    radialWeight = exp(-2.0 .* rNorm) .* ...
        (0.65 + 0.35 .* sin(2*pi*tau + theta).^2);
...
metrics.field_height = H;
metrics.field_width = Wd;
metrics.time_steps = T;
metrics.selected_dataset_count = numel(selected);
metrics.metric_mean = nanmean_local(metric(:));
metrics.metric_std = nanstd_local(metric(:));
metrics.velocity_magnitude_mean = nanmean_local(velocityMagnitude(:));
metrics.velocity_magnitude_max = max_finite(velocityMagnitude(:));
metrics.final_scalar_mean = nanmean_local(F(:));
metrics.final_scalar_std = nanstd_local(F(:));
...
function selected = select_ssot_datasets(datasets)
    ...
    for k = 1:numel(datasets)
        ...
        pRaw = datasets(k).path;

        if ischar(pRaw)
            p = lower(pRaw);
        else
            p = lower(char(string(pRaw)));
        end
...
function TBL = run_projection_law_tests(Vx, Vy, x, y, Fx, Fy)
    ...
    caseNames = { ...
        'CASE_1_H5_self_aligned_velocity', ...
        'CASE_2_H5_radial_direction', ...
        'CASE_3_H5_gradient_direction' ...
    };
    ...
    for i = 1:numel(caseNames)
        ...
        rows(end+1) = make_projection_row(caseName, 'H1_scalar_coef_equals_speed2', scalarCoef, speed2); %#ok<AGROW>
        rows(end+1) = make_projection_row(caseName, 'H2_shadow_norm2_equals_speed2', shadowNorm2, speed2); %#ok<AGROW>
        rows(end+1) = make_projection_row(caseName, 'H3_signed_component_squared_equals_speed2', signedComponentSquared, speed2); %#ok<AGROW>
    end
...
function report = test_geodesic_velocity_squared_vs_light( ...
    ...
    if ~cal.ok
        ...
        if isempty(dimless_vg2)
            report.dimensionless_vg2_max = NaN;
            report.dimensionless_vg2_p99 = NaN;
        else
            report.dimensionless_vg2_max = max(dimless_vg2);
            report.dimensionless_vg2_p99 = percentile_local(dimless_vg2, 99);
        end

        report.conclusion = ...
            'RESULT: H5 field was computed, but c^2 comparison was not physically evaluated because dx_m and dt_s were not found inside the H5.';
    ...
    if isempty(ratioVals)
        report.status = 'NO_VALID_RATIO_VALUES';
        report.conclusion = 'RESULT: no valid v_g^2/c^2 ratio values were computed.';
        return;
    end

    ratio_max = max(ratioVals);
    ratio_p99 = percentile_local(ratioVals, 99);
    ratio_p95 = percentile_local(ratioVals, 95);
...
function value = scan_attrs(h5Path, objPath, attrs, candidateNames)
    ...
    for i = 1:numel(attrs)
        ...
        for j = 1:numel(candidateNames)
            if strcmpi(attrName, candidateNames{j})
